...
<script setup lang="ts">
  ...
  import { graphJSON, layoutJSON, foldChangesByType, foldChangesByID, reactomeEntry } from '../core/reactomeTypes'
  ...
  const currentInsetPathwaysTotals = ref({} as {[key: number] : {proteomics : number, metabolomics : number, transcriptomics : number}})
  ...
  const getJsonFiles  = ((reactomeID: string) => {
    ...
    fetch(`/get_reactome_json_files/${reactomeID}`, {
      ...
      .then((dataContent) => {
        ...
        currentInsetPathwaysTotals.value = dataContent.insetPathwayTotals as {[key: number]: {proteomics: number, metabolomics: number, transcriptomics: number}}
        drawDetailView()
      }).then(() => $q.loading.hide())
  })
  /**
   ...
   */
  const prepareFcs = ((pathwayData: any, fcs: foldChangesByType, fcsReactomeKey:foldChangesByID, type: 'proteomics' | 'metabolomics' | 'transcriptomics') => {
  ...
  const drawDetailView = (() => {
    ...
    const pathwayData = overviewData.value.find((elem: { pathwayId: string }) => elem.pathwayId === pathwaySelection.value.value)
    ...
    currentView.value = new ReactomeDetailView(currentLayoutJson.value, currentGraphJson.value, '#' + props.contextID, fcs, fcsReactomeKey)
  })
