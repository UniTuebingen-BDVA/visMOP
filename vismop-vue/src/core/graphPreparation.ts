...
function generateColorScale(min_val: number, max_val: number) {
  ...
  return function (val: number) {
    ...
    if (isNaN(val)) {
      ...
      const rRGB =
        200 - Math.round(val * stepsize_smaller0) < 0
          ? 0
          : 200 - Math.round(val * stepsize_smaller0);
      const gRGB =
        200 - Math.round(val * stepsize_smaller0) < 0
          ? 0
          : 200 - Math.round(val * stepsize_smaller0);
      ...
      const rRGB = 255;
      const gRGB =
        200 - Math.round(val * stepsize_larger0) < 0
          ? 0
          : 200 - Math.round(val * stepsize_larger0);
      const bRGB =
        200 - Math.round(val * stepsize_larger0) < 0
          ? 0
          : 200 - Math.round(val * stepsize_larger0);
