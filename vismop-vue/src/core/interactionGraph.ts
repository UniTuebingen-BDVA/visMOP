 ...
 * @returns
 */
function generatePieCharts(identities: number[]): {
  [key: string]: { [key: string]: string };
} {
  const pieCharts: { [key: string]: { [key: string]: string } } = {
    saturated: {},
    faded: {},
  };
  ...
  combinations.forEach((elem) => {
    ...
    const pieSVG = generateSinglePieChart(
      elem.length,
      elem.map((idx) => satColors[idx]),
      64
    );
...
function combinationOfArray(arr: number[]): number[][] {
  if (arr.length === 1) return [arr];
    ...
    return subarray.concat(
      subarray.map((e) => e.concat(arr[0])),
      [[arr[0]]]
    );
  }
}

function generateSinglePieChart(
  segments: number,
  colors: string[],
  diameter: number
): SVGElement {
