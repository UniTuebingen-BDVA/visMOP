...
import { graphData, nodeAttr } from "@/core/graphTypes";
...
export function layoutToPathway(
  ...
  const cameraCenterPos = get_pathway_center_pos(
    renderer,
    graph,
    nodeIDs,
    "camera"
  ...
  const graphCenterPos = renderer.viewportToGraph(
    get_pathway_center_pos(renderer, graph, nodeIDs, "graph")
  );
 ...
 * @returns
 */
function get_pathway_center_pos(
  renderer: Sigma,
  graph: Graph,
  nodeIDs: string[],
  type: "graph" | "camera"
