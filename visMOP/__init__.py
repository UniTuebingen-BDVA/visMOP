...
from visMOP.python_scripts.keggAccess import (
    gene_symbols_to_keggID,
    multiple_query,
    kegg_get,
    parse_get,
    get_unique_pathways,
    query_kgmls,
    associacte_value_keggID,
)
...
def kegg_parsing():
        ...
        try:
            ...
            for ID in prot_dict_global:
                ...
                try:
                    ...
                    fold_changes[entry["keggID"]] = {
                        "transcriptomics": "NA",
                        "proteomics": entry[proteomics["value"]],
                        "metabolomics": "NA",
                    }
                except:
    ...
    if metabolomics["recieved"]:
        ...
        metabolomics_df = metabolomics_df_global.drop_duplicates(
            subset=metabolomics["symbol"]
        ).set_index(metabolomics["symbol"])
        ...
        for elem in metabolomics_keggIDs:
            if elem in fold_changes:
                fold_changes[elem]["metabolomics"] = metabolomics_dict[elem][
                    metabolomics["value"]
                ]
                ...
                fold_changes[elem] = {
                    "transcriptomics": "NA",
                    "proteomics": "NA",
                    "metabolomics": metabolomics_dict[elem][metabolomics["value"]],
                }
    ...
    if transcriptomics["recieved"]:
        ...
        transcriptomics_df = transcriptomics_df_global.drop_duplicates(
            subset=transcriptomics["symbol"]
        ).set_index(transcriptomics["symbol"])
        ...
        gene_symbols_transcriptomics = [
            symb
            for symb in gene_symbols_transcriptomics
            if symb not in unwanted_temporary
        ]
        (
            transcriptomics_keggIDs,
            symbol_kegg_dict_transcriptomics,
            ...
            gene_symbols_transcriptomics,
            target_db,
            data_path / "kegg_cache/gene_symbol_cache.json",
        )
        for symbol in gene_symbols_transcriptomics:
            try:
                keggID = symbol_kegg_dict_transcriptomics[symbol]
                if keggID in fold_changes:
                    fold_changes[keggID]["transcriptomics"] = transcriptomics_dict[
                        symbol
                    ][transcriptomics["value"]]
                else:
                    fold_changes[keggID] = {
                        "transcriptomics": transcriptomics_dict[symbol][
                            transcriptomics["value"]
                        ],
                        "proteomics": "NA",
                        "metabolomics": "NA",
                ...
                print(
                    "Symbol: {} was not in the kegg dictionary, probably due to it not being found in the kegg DB"
                )
    ...
    kegg_gets = kegg_get(
        keggIDs=combined_keggIDs, caching_path=data_path / "kegg_cache/kegg_gets.json"
    )
    ...
    pathway_amount_dict = {
        k: v for k, v in (pathway.return_amounts() for pathway in parsed_pathways)
    }
        ...
        if not global_dict_entries[key]["name"] is None:
            for i in range(len(global_dict_entries[key]["name"])):
                if global_dict_entries[key]["name"][i].startswith("TITLE:"):
                    global_dict_entries[key]["name"][i] = global_dict_entries[key][
                        "name"
                    ][i][6::]
    ...
    print(
        f"From {len(global_dict_entries)} entries in total, {len(without_empty)} remain after filtering entries without useful edges"
    )
    ...
    out_dat = {
        ...
        "pathwayLayouting": {
            "pathwayList": dropdown_pathways,
            "pathwayNodeDictionary": pathway_node_dict,
        },
