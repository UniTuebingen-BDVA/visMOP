...
def get_networkx_with_edge_weights_all_nodes_connected(
    pathway_info_dict, stringGraph, use_brite, use_interaction
):
    ...
    for nodeA in pathway_info_dict.keys():
        ...
        for nodeB in pathway_info_dict.keys():
            if nodeB not in nodes_done:
                brite_score, interaction_score = calculate_edge_weight(
                    pathway_info_dict[nodeA],
                    pathway_info_dict[nodeB],
                    stringGraph,
                    use_brite,
                    use_interaction,
                )
    ...
    min_val_interaction, max_val_interaction = min(interaction_scores), max(
        interaction_scores
    )
    total_scores = [
        normalize_in_range(brite_score, min_val_brite, max_val_brite, [0, 1])
        + normalize_in_range(
            interaction_score, min_val_interaction, max_val_interaction, [0, 1]
        )
        for brite_score, interaction_score in zip(brite_scores, interaction_scores)
    ]
    ...
    for nodes, w in zip(node_names, total_scores):
        norm_w = (
            normalize_in_range(w, min_val_total, max_val_total, [0, 10])
            if use_brite and use_interaction
            else w
        )
...
def calculate_edge_weight(pathway1, pathway2, stringGraph, use_brite, use_interaction):
    ...
    interaction_score = 0
    if use_brite:
        for importance, brite_hier_level in enumerate(
            [
                "brite_hier_superheadings",
                "brite_hier_subcategories",
                "brite_hier_proteinIDs",
            ]
        ):
            if (
                len(pathway1[brite_hier_level]) > 0
                and len(pathway2[brite_hier_level]) > 0
            ):
                val_ratio_path_1 = sorted(
                    [
                        val / pathway1["numEntries"]
                        for val in pathway1[brite_hier_level].values()
                    ]
                )
                val_ratio_path_2 = sorted(
                    [
                        val / pathway2["numEntries"]
                        for val in pathway2[brite_hier_level].values()
                    ]
                )
    ...
    if len(proteins_p1) > 0 and use_interaction:
        ...
        for prot in proteins_p1:
            interactingProt_prot1.update(
                {
                    edgeProts[1]: edgeProts[2]["weight"]
                    for edgeProts in stringGraph.filtered_graph.edges(prot, data=True)
                }
            )
        interactingProt_prot2 = list(
            set(interactingProt_prot1.keys()) & set(proteins_p2)
        )
        if len(interactingProt_prot2) > 0:
            all_interaction_scores = [
                interactingProt_prot1.get(key) for key in interactingProt_prot2
            ]
