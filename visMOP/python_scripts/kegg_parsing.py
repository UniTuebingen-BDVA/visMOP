...
from visMOP.python_scripts.kegg_pathway import (
    KeggPathway,
    KeggPathwayEntry,
    KeggPathwayRelation,
    KeggPathwayReaction,
        ...
        if current_entry.entryType == "group":
            ...
            if kgml_graphics.get("x") and kgml_graphics.get("y"):
                current_entry.add_origPos(
                    pathway_ID,
                    [int(kgml_graphics.get("x")), int(kgml_graphics.get("y"))],
                )
            # testing
            else:
    ...
    for reaction in root.findall("reaction"):
        ...
        for substrate in reaction.findall("substrate"):
            ...
            current_reaction_part = KeggPathwayReaction(
                substrate_keggID, r_elem, r_type, pathway_ID, pathway.title
            ).asdict()
        ...
        for product in reaction.findall("product"):
            ...
            global_entry[r_elem].is_empty = False
            current_reaction_part = KeggPathwayReaction(
                r_elem, product_keggID, r_type, pathway_ID, pathway.title
            ).asdict()
