...
def parse_KGML(pathway_ID, kgml, global_entry, global_relation, global_reaction, value_dict):
    ...
    for entry in root.findall('entry'):
        ...
        if entry_keggID in global_entry.keys():
            ...
            for keggID in entry_keggID_list:
                ...
                try:
                    ...
                    value = value_dict[keggID.replace("cpd:", "").replace(
                        "glu:", "").replace("ko:", "").replace("dr:", "dr")]
                    # print(value)
                except:
                    if not value:
                        value = {"transcriptomics": "NA",
                                 "proteomics": "NA", "metabolomics": "NA"}
        ...
        if current_entry.entryType == "group":
            ...
            if(kgml_graphics.get('x') and kgml_graphics.get('y')):
                current_entry.add_origPos(
                    pathway_ID, [int(kgml_graphics.get('x')), int(kgml_graphics.get('y'))])
            # testing
            else:
        ...
        if (current_entry_1 in groups.keys()) and (current_entry_2 in groups.keys()):
            ...
            for group_ID1 in group_IDs1:
                for group_ID2 in group_IDs2:
                    ...
                    current_relation = KeggPathwayRelation(
                        current_keggID_1, current_keggID_2, current_type, current_subtypes, pathway_ID, pathway.title).asdict()
                    ...
                    global_entry[current_keggID_1].add_outgoing(
                        current_relation)
                    global_entry[current_keggID_1].add_pathway_edge(
                        pathway_ID, current_keggID_1, current_keggID_2)
    ...
    for reaction in root.findall('reaction'):
        ...
        for substrate in reaction.findall('substrate'):
            ...
            current_reaction_part = KeggPathwayReaction(
                substrate_keggID, r_elem, r_type, pathway_ID, pathway.title).asdict()
        ...
        for product in reaction.findall('product'):
            ...
            global_entry[r_elem].is_empty = False
            current_reaction_part = KeggPathwayReaction(
                r_elem, product_keggID, r_type, pathway_ID, pathway.title).asdict()
