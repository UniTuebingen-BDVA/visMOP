...
def generate_pickles(file_path, mapping_file):
    ...
    with open(data_path / mapping_file) as fh:
        for line in fh:
            ...
            if organism not in databse_2_reactome:
                databse_2_reactome[organism] = {}
            if uniprot_ID in databse_2_reactome[organism]:
                if reactome_entity_ID in databse_2_reactome[organism][uniprot_ID]:
                    databse_2_reactome[organism][uniprot_ID][reactome_entity_ID]['pathways'].append((reactome_pathway_ID, reactome_pathway_Name))
                else:
                    databse_2_reactome[organism][uniprot_ID][reactome_entity_ID] = {'reactome_id':reactome_entity_ID, 'name': entity_name, 'pathways': [(reactome_pathway_ID, reactome_pathway_Name)]}
            else:
                databse_2_reactome[organism][uniprot_ID] = {}
                databse_2_reactome[organism][uniprot_ID][reactome_entity_ID] = {'reactome_id':reactome_entity_ID, 'name': entity_name, 'pathways': [(reactome_pathway_ID, reactome_pathway_Name)]}
    for key in databse_2_reactome.keys():
