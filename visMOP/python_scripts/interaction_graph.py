...
class StringGraph:
            ...
            for line in f:
                if(line[0] != 'p'):
                    ...
                    if int(splitline[2]) > 400:
                        lines.append(line)
            self.complete_graph = nx.parse_edgelist(
                lines, create_using=nx.Graph, comments="protein1", data=(("weight", int),))
    ...
    def filter_by_confidence(self, weight_threshold):
        ...
        edges_to_remove = [(source, target) for source, target, weight in copy_graph.edges(
            data="weight") if weight < weight_threshold]
    ...
    def query_ego_graph(self, node, filtered_graph, ego_index, radius, name_dict):
        ...
        ego_graph = nx.ego_graph(filtered_graph, node,
                                 undirected=False, radius=radius)

        nodes_to_update = {node: name_dict[node]
                           for node in ego_graph.nodes if node in name_dict}
        ...
        edges_to_update = {(u[0], u[1]): ego_index for u in ego_graph.edges if (
            u[0] == node or u[1] == node)}
    ...
    def get_merged_egoGraph(self, string_IDs, radius, name_dict, weight_threshold):
        ...
        ego_graphs = {node: self.query_ego_graph(
            node, filtered_graph, idx, 1, name_dict) for idx, node in enumerate(string_IDs)}
        ...
        """
        egoID_identity = {egoID: idx for idx,
                          egoID in enumerate(list(ego_graphs.keys()))}
        identity_egoID = {idx: egoID for idx,
                          egoID in enumerate(list(ego_graphs.keys()))}

        composed_graph.graph["egoID_identity"] = egoID_identity
        composed_graph.graph["identity_egoID"] = identity_egoID
        composed_graph.graph["identities"] = list(
            range(len(list(ego_graphs.values()))))
        print("completeGraph", nx.info(composed_graph))
        intersections_node = self.calculate_intersections_nodes(
            composed_graph, ego_graphs)
        intersections_edge = self.calculate_intersections_edges(
            composed_graph, ego_graphs)
    ...
    def calculate_ego_ego_edges(self, graph):
        ...
        for edge in graph.edges:
            if((edge[0] in egoIDs) and (edge[1] in egoIDs)):
                ...
                ego_edges_color[edge] = [graph.graph["egoID_identity"]
                                         [edge[0]], graph.graph["egoID_identity"][edge[1]]]
    ...
    def print_info(self):
        ...
        with open('out.txt', 'w') as f:
            print('FilteredVerbatim:', list(
                self.filtered_graph.edges(data="weight")), file=f)
