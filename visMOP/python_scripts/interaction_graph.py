...
class StringGraph:
    ...
    def filter_by_confidence(self, weight_threshold):
        ...
        edges_to_remove = [
            (source, target)
            for source, target, weight in copy_graph.edges(data="weight")
    ...
    def query_ego_graph(self, node, filtered_graph, ego_index, radius, name_dict):
        ...
        nodes_to_update = {
            node: name_dict[node] for node in ego_graph.nodes if node in name_dict
        }
        ...
        edges_to_update = {
            (u[0], u[1]): ego_index
            for u in ego_graph.edges
            if (u[0] == node or u[1] == node)
        }
    ...
    def get_merged_egoGraph(self, string_IDs, radius, name_dict, weight_threshold):
        ...
        ego_graphs = {
            node: self.query_ego_graph(node, filtered_graph, idx, 1, name_dict)
            for idx, node in enumerate(string_IDs)
        }
        ...
        """
        egoID_identity = {
            egoID: idx for idx, egoID in enumerate(list(ego_graphs.keys()))
        }
        identity_egoID = {
            idx: egoID for idx, egoID in enumerate(list(ego_graphs.keys()))
        }
    ...
    def calculate_ego_ego_edges(self, graph):
        ...
        for edge in graph.edges:
            if (edge[0] in egoIDs) and (edge[1] in egoIDs):
                ...
                ego_edges_color[edge] = [
                    graph.graph["egoID_identity"][edge[0]],
                    graph.graph["egoID_identity"][edge[1]],
                ]
    ...
    def print_info(self):
        ...
        with open("out.txt", "w") as f:
            print(
                "FilteredVerbatim:",
                list(self.filtered_graph.edges(data="weight")),
                file=f,
            )
