...
def NetworkSmoothing(graph, data_tabel, eigenvec_matrix):
    ...
    if eigenvec_matrix is None:
        ...
        la_placian_matrix = nx.laplacian_matrix(
            graph, nodelist=node_names, weight=None
        ).asfptype()
        _, eigenvec_matrix = eigs(la_placian_matrix.toarray(), k=len(data_tabel.index))
...
def project_vector_on_first_components_of_a_basis(
    vec, basis, num_of_first_vectors, start_from
):
...
def fill_missing_values_with_neighbor_mean(
    graph_dict, data_table, recieved_omics, defaul_means, numValsPerOmic
):  # defold values per omic and pos
    defaul_means_rec_omic = [
        default_mean
        for (default_mean, recieved) in zip(defaul_means, recieved_omics)
        if recieved
    ]
    ...
    for node_name in node_names:
        ...
        for pos, val in enumerate(data_table.loc[node_name]):
            default_val = (
                ...
                if pos % 1 != 0
                or pos % 2 != 0
                or pos // numValsPerOmic > len(defaul_means_rec_omic) - 1
                else defaul_means_rec_omic[pos // numValsPerOmic]
            )
            new_val = val
            if math.isnan(val):
...
def get_pca_layout_pos(data_table):
    ...
    pos_dic_pca = {
        node_name: row for row, node_name in zip(new_pos, list(data_table.index))
    }
...
def normalize_all_x_y_to_ndc(pos_per_node, val_space):
    ...
    norm_vals = {
        node: [
            normalize_in_range(xy[0], min_x, max_x, val_space),
            normalize_in_range(xy[1], min_y, max_y, val_space),
        ]
        for node, xy in pos_per_node.items()
    }
    return norm_vals
...
def morph_layouts(xy_1, xy_2, percentage):
    new_xy = {
        node: coordinate_in_morphed_graph(xy_1[node], xy_2[node], percentage)
        for node in list(xy_1.keys())
    }
    return new_xy
