 ...
 */
 ...
 function drawLabel(context: CanvasRenderingContext2D, data: PartialButFor<NodeDisplayData, "x" | "y" | "size" | "label" | "color">, settings: Settings):void {
    ...
    labelText.forEach(
        (row,i) => {
            context.fillText(row, data.x + data.size + 3, data.y + (size / 3) + (i- Math.floor(labelText.length/2)) * size);
        }
    )
}
 /**
   ...
   if (typeof data.label === "string") {
    ...
    splitText.forEach(
     ...
     const  boxWidth = Math.round(textWidth + 9),
       boxHeight = Math.round((size * splitText.length )+ 2 * MARGIN),
       radious = Math.max(data.size, (size * splitText.length) / 2) + MARGIN;
